...
ソース管理について、Terrain用テクスチャのサイズが100MBを超過してしまったため、当初はGitによるソース管理は行わずローカルで管理を行っていましたが、完成間際でGit LFSを知り、それを利用することで可能にしました。

このことから、Gitはコード管理には適するものの、3Dモデルや高解像度のテクスチャ管理には、LFS等の拡張や管理方法の見直しが必要だということを再認識しました。
...
歩いていると突然前方に泣き声を発する日本人形が出現します。一定距離近づくと画面全体にジャンプスケアが発生します。
